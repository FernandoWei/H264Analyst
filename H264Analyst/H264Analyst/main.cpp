...
//    std::string url = "http://10.11.10.45/ttkankan/video/test3.flv";
    ...
    std::unique_ptr<H264Analyst::MediaSource, std::function<void(H264Analyst::MediaSource* source)> > mediaSource(new H264Analyst::MediaSource(std::move(std::string(url))), [](H264Analyst::MediaSource* source){
        return source->close();
    });
